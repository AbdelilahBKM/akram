  ...
  useEffect(() => {
    if (product && listProduits.length) {
      const filterProduct = listProduits
        .filter(
          (item) => item.categorie.id === product.categorie.id && item.id !== product.id
        )
        .slice(0, 4);
      ...
      <section className="grid md:grid-cols-2 gap-6 lg:gap-12 items-start max-w-6xl px-4 mx-auto py-20 mt-5">
          ...
          <div className="grid gap-3 md:grid-cols-5 md:gap-3">
            <div className="md:col-span-4">
              {product?.image_produits && (
                <Image
                  src={`http://127.0.0.1:8000/storage/images/${product.image_produits}`}
                  alt="Product Image"
                  width={600}
                  height={900}
        ...
        <div className="grid gap-4 md:gap-10 items-start">
          <div className="grid gap-2">
            <h1 className="font-bold text-3xl lg:text-4xl">{product?.nom_produit}</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {relatedProducts.length > 0 ? (
            relatedProducts.map((prod) => (
              <div key={prod.id} className="bg-background rounded-lg shadow-sm overflow-hidden">
                <Image
                  src={`http://127.0.0.1:8000/storage/images/${prod.image_produits}`}
                  alt="Related Product"
                  width={400}
                  height={400}
                ...
                <div className="p-4 space-y-2">
                  <Link href={`/produits/${prod.id}`} className="text-lg font-semibold hover:underline cursor-pointer">
                    {prod.nom_produit}
                  </Link>
