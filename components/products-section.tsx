...
export default function Product_section() {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        setProduitsAfficher(data.filter((prod: Produits) => prod.categorie.id === activeCategory));
        setIsLoading(false);
      } catch (error) {
  ...
  useEffect(() => {
    setProduitsAfficher(listProduits.filter(prod => prod.categorie.id === activeCategory));
  ...
  const productsToDisplay = showAll ? produitsAfficher : produitsAfficher.slice(0, 8);
            ...
            <ul className="flex space-x-4 text-slate-700">
              {listCategories.map((category) => (
                <li key={category.id}>
                  <div>
                    <button
                      className={`px-4 py-2 rounded-full ${activeCategory === category.id ? "bg-white font-semibold shadow " : "hover:bg-gray-200"
                        }`}
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {productsToDisplay.map((produit) => (
                  <div key={produit.id} className="rounded-lg bg-card shadow-md cursor-pointer">
                    ...
                    />
                    <div className="p-4">
                      <Link href={"/produits/" + produit.id} className="text-lg font-semibold text-card-foreground hover:underline transition-all">
                        {produit.nom_produit}
                      </Link>
